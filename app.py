...
def truncate_personal_fields():
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("""
        UPDATE users
        SET name = 'ANONYM',
            email = 'anonym@example.local'
    """)
    conn.commit()
    conn.close()
    print("All personal fields have been anonymized with placeholders (GDPR compliant)")
